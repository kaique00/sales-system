...
export default function Account() {
    ...
    async function salvaItem(accountId, productId, productValue){
        await api.post('itensaccount', {account_id: accountId, product_id: productId, value:productValue})
    }
    async function finalizar(e) {
       const account = await api.post('accounts', {value: total, user_id:1})
        listProducts.map(product => (
            salvaItem(account.id,product.id, product.value)
           ))
